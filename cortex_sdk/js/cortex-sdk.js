...
var cortex_sdk = 
{
    ...
    utils: {
        verify_address: function(address, chain, network = 'public')
        {
            var btc_js_chain = 'bitcoin';
            var valid_address = false;
            if(address && chain)
            {
                if(chain == 'btc' || chain == 'Bitcoin' || chain == 'bitcoin')
                {
                    if(network == 'private') btc_js_chain = 'bitcointestnet';
                    try
                    {
                        if(nwbs.bitcoin.address.toOutputScript(address, bitcoin.networks[btc_js_chain])) valid_address = true;
                    }
                    catch(e)
                    {
                ...
                else if(chain == 'ltc' || chain == 'Litecoin' || chain == 'litecoin')
                {
                    btc_js_chain = 'litecoin';
                    if(network == 'private') btc_js_chain = 'litecointestnet';
                    try
                    {
                        if(nwbs.bitcoin.address.toOutputScript(address, bitcoin.networks[btc_js_chain])) valid_address = true;
                    }
                ...
                else if(chain == 'eth' || chain == 'Ethereum' || chain == 'ethereum')
                {
                    if(EthJS.Util.isValidAddress(address)) valid_address = true;
                }
                else if(chain == 'xrp' || chain == 'Ripple' || chain == 'ripple')
                {
                    if(rippled.RippleAPI.isValidClassicAddress(address)) valid_address = true;
                }
    ...
    forms: 
    {
        ...
        {
            var forms = document.getElementsByClassName(cortex_sdk.classes.registration);
            if(forms)
            {
                for(i = 0; i < forms.length; i++)
                {
